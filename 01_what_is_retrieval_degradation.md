...
When you build an AI system that answers questions, the system has to **find** the right information before it can **answer** with it. That finding process is called retrieval.

**Retrieval degradation** is when the system finds the wrong information — or finds outdated information, or finds information that's close but not quite right — and then answers confidently based on it. The answer sounds correct. It isn't.

---

## Why this matters for SMB builders specifically

Enterprise builders have QA teams, staging environments, and users who can usually identify when something looks wrong. SMB clients typically have none of these.

When retrieval degrades in an SMB deployment:
...
- The builder finds out weeks later, usually from a frustrated client

**The builder is responsible for catching this before handoff. The client cannot.**
...
**Example:** A customer asks "Is the matte black 3M wrap in stock?" A vector search returns a product description for a similar matte wrap that is in stock — even though the specific product asked about is not. The system answers "yes." The order gets placed. The product doesn't exist in inventory.
...
**Example:** A 2023 pricing sheet and a 2024 pricing sheet are both in the vector store. A customer asks about pricing. The system blends both into a confident answer. The quoted price is from the old sheet. The client has an uncomfortable conversation.
